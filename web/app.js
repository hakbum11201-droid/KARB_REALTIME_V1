...
function renderExecutionPlan(plan={}) {
  ...
  const fields = [
    ...
    ['Buy Leg Age', `${fmt(plan.buy_leg_quote_age_ms,0)} ms`], ['Sell Leg Age', `${fmt(plan.sell_leg_quote_age_ms,0)} ms`],
    ['Leg Age Cap', `${fmt(plan.leg_quote_age_cap_ms,0)} ms`], ['Leg Freshness', String(Boolean(plan.leg_freshness_ok))],
    ['Leg Freshness Blocker', plan.leg_freshness_blocker||'none'], ['Entry Decision Wait', `${fmt(plan.entry_decision_wait_ms,0)} ms`],
