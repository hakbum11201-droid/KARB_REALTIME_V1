...
function renderStaleRecheckTelemetry(t={}) {
  ...
  const actionableRatio=Number(t.stale_recheck_actionable_fast_pass_ratio||0)*100;
  const timeoutRatio=Number(t.stale_recheck_timeout_ratio||0)*100;
  const topSymbols=(t.stale_recheck_by_symbol||[])
    .slice(0,3)
    .map(row=>`${row.symbol}:${fmt(row.actionable_fast_pass_count)}`)
    .join(', ')||'--';
  el.textContent=`Stale Recheck Health ${t.stale_recheck_health||'WATCH'} | Enabled ${String(Boolean(t.stale_recheck_enabled??true))} | Paper Only ${String(Boolean(t.stale_recheck_paper_only??true))} | Requests ${fmt(t.stale_recheck_request_count)} | Execute ${fmt(t.stale_recheck_execute_count)} | Pass / Fail / Timeout ${fmt(t.stale_recheck_pass_count)} / ${fmt(t.stale_recheck_fail_count)} / ${fmt(t.stale_recheck_timeout_count)} | FAST PASS ${fmt(t.stale_recheck_fast_pass_count)} | LATE PASS ${fmt(t.stale_recheck_late_pass_count)} | Actionable Fast Pass ${fmt(t.stale_recheck_actionable_fast_pass_count)} | WIDE_SPREAD RECHECK ${fmt(t.paper_wide_spread_recheck_request_count)} / ${fmt(t.paper_wide_spread_recheck_actionable_count)} | WIDE_SPREAD ENTRY ${fmt(t.paper_wide_spread_recheck_entry_count)} ${t.paper_wide_spread_recheck_last_symbol||'--'} | Actionable Ratio ${fmt(actionableRatio,1)}% | Top Symbols ${topSymbols} | Timeout Ratio ${fmt(timeoutRatio,1)}% | Avg New Surplus ${fmt(t.stale_recheck_avg_new_surplus_bp,1)} bp | FAST PASS RATIO ${fmt(fastRatio,1)}% | LATE PASS RATIO ${fmt(lateRatio,1)}% | Pass Ratio ${fmt(passRatio,1)}% | Queue ${fmt(t.stale_recheck_queue_size)} | Handoff Match ${fmt(t.stale_recheck_completed_handoff_pending_match_count)} / ${fmt(t.stale_recheck_completed_handoff_count)} | Fetch Avg ${fmt(t.stale_recheck_avg_handoff_fetch_ms,1)} ms | Decision Wait ${fmt(t.stale_recheck_avg_handoff_decision_wait_ms,1)} ms | Priority Wakes ${fmt(t.stale_recheck_priority_worker_wake_count)} | Symbol Fetches ${fmt(t.stale_recheck_priority_symbol_fetch_count)} | Full Refresh Fallback ${fmt(t.stale_recheck_priority_full_refresh_fallback_count)} | Fetch Avg ${fmt(t.stale_recheck_priority_fetch_avg_ms,1)} ms | Inflight ${fmt(t.stale_recheck_inflight_count)} | Deduped ${fmt(t.stale_recheck_deduped_count)} | Last ${t.stale_recheck_last_symbol||'--'} ${t.stale_recheck_last_status||'NONE'} | AVG TOTAL ${fmt(t.stale_recheck_avg_total_elapsed_ms??t.stale_recheck_avg_elapsed_ms,1)} ms | AVG QUEUE ${fmt(t.stale_recheck_avg_queue_wait_ms,1)} ms | AVG FETCH ${fmt(t.stale_recheck_avg_fetch_ms,1)} ms | AVG DECISION WAIT ${fmt(t.stale_recheck_avg_decision_wait_ms,1)} ms`;
}
...
function renderRestFallbackCacheStatus(t={}) {
  ...
  if (cacheEl) {
    ...
    cacheEl.textContent=`REST Fallback Cache: ${cache.enabled?'ON':'OFF'} | Running ${cache.running?'YES':'NO'} | Quote Count ${fmt(cache.quote_count)} | Stale Count ${fmt(cache.stale_count)} | Hit Count ${fmt(t.rest_fallback_cache_hit_count)} | Miss Count ${fmt(t.rest_fallback_cache_miss_count)} | Direct REST Calls ${fmt(t.rest_direct_call_count)} | Older-than-WS Drops ${fmt(t.rest_fallback_older_than_ws_drop_count)} | Upbit REST cache/fx/other ${fmt(t.upbit_rest_call_count_rest_cache)}/${fmt(t.upbit_rest_call_count_fx)}/${fmt(t.upbit_rest_call_count_other)} | 429 Source cache/fx/other ${fmt(t.upbit_429_count_rest_cache)}/${fmt(t.upbit_429_count_fx)}/${fmt(t.upbit_429_count_other)} | WS Fresh Skip ${fmt(cache.upbit_ws_fresh_skip_count)} | Bithumb Skipped Last Loop ${fmt(t.skipped_bithumb_symbol_count_last_loop)} | Quote Age P95 ${fmt(t.p95_quote_age_ms,1)} ms | Fetch Latency P95 ${fmt(t.p95_quote_fetch_latency_ms,1)} ms | Stale Symbol Ratio ${fmt(Number(t.stale_symbol_ratio||0)*100,1)}% | FX Cache Age ${fxAge} | FX Status ${t.fx_status||'--'} | FX Last Error ${t.fx_last_error||'none'}`;
  }
}
